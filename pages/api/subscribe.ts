...
async function updateSubscription(db, subscription: Subscription) {
  ...
  const stream = await streamCollection.findOne({service_key: subscription.service_key});
...
async function adjustSubscription(request: NextApiRequest, response: NextApiResponse) {
