...
export class DigestManager {
  public static DAY_IN_MS = 86400000;

  public stream: StreamProps;
  public db: any;
  public dryRun: boolean;
  public cache: { [date: string]: boolean };
  ...
  public async createDigests(processDate: Date): Promise<DigestProps[]> {
    ...
    while (await cursor.hasNext()) {
      ...
      const modifiedLastDigest =
        lastDigest === undefined
          ? new Date(
              processDate.getTime() -
                subscription.frequency * 60 * 60 * 24 * 1000
            )
      ...
      if (
        ...
        const contentAvailable = await this.hasContent(
          processDate.getTime(),
          modifiedLastDigest.getTime()
        );
        if (contentAvailable) {
  ...
  public async completeDigest(digest: DigestProps) {
    const subscriptionsCollection = this.db.collection("subscriptions");
  ...
  public async recordDigests(digests: DigestProps[]): Promise<number> {
    if (digests.length === 0) {
      return 0;
    }
  ...
  public async hasContent(
    ...
    const filter = {
      ...
      date_fetched: { $gt: new Date(lastDate), $lt: new Date(processDate) }
    };
